...
FROM     srr_info LEFT OUTER JOIN
                  srr_status ON srr_info.srr_status_cd = srr_status.srr_status_cd
WHERE  (srr_info.lgd_completed_dt > '#datefrom#') AND (srr_info.lgd_completed_dt < '#dateto#')
</cfquery>
...
<form action="report_dateentry.cfm" method="post" name="reports" id="reports" onSubmit="return checkForm()">
<input type="hidden" name="date_from" value="<cfoutput><cfif #request.date_from# is not"">#request.date_from#</cfif></cfoutput>">
<input type="hidden" name="date_to" value="<cfoutput><cfif #request.date_to# is not"">#request.date_to#</cfif></cfoutput>">
<DIV ALIGN="center"><input type="submit" name="submit_daterange" id="submit_daterange" value="Back"></DIV>
</form>
</cfoutput>
