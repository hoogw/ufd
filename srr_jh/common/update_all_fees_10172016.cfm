...
<cfloop index="i" from="1" to="#max_no.max_no#" step="1">
	...
	select * from driveway_details
	where ref_no=#request.ref_no# and driveway_no=#i#
	</cfquery>
	
	<cfif #get_driveway.recordcount# is not 0>
	
	<cfif #get_driveway.driveway_material# eq "c">
	<cfset driveway_fee=#get_driveway.driveway_qty#*#get_rates.conc_driveway_approach_uf#>
	<cfelseif #get_driveway.driveway_material# eq "a">
	<cfset driveway_fee=#get_driveway.driveway_qty#*#get_rates.street_resurfacing_uf#>
	</cfif>
	
	<cfif #get_driveway.waive_driveway_fee# is "Y">
	<cfset driveway_fee_discount=#driveway_fee#>
	<!--cfelseif #get_driveway.driveway_fee_discount# is not "" and #get_driveway.driveway_fee_discount# is not 0-->
	<!--cfset driveway_fee_discount=#get_driveway.driveway_fee_discount#-->
	<cfelse>
	<cfset driveway_fee_discount=0>
	</cfif>
	
	
	<cfif #driveway_fee# is "">
	<cfset driveway_fee=0>
	</cfif>
	
	<cfset driveway_net_fee=#driveway_fee#-#driveway_fee_discount#>
	
	<cfset driveway_net_fee=#decimalformat(driveway_net_fee)#>
	<cfset driveway_net_fee=ReplaceNoCase("#driveway_net_fee#",",","","ALL")>
	
	<cfset driveway_s1=(#driveway_net_fee#)*(#get_rates.surcharge_1#/100)>
	<cfset driveway_s2=(#driveway_net_fee#)*(#get_rates.surcharge_2#/100)>
		
	<cfset driveway_s1=#decimalformat(driveway_s1)#>
	<cfset driveway_s1=ReplaceNoCase("#driveway_s1#",",","","ALL")>
	
	<cfset driveway_s2=#decimalformat(driveway_s2)#>
	<cfset driveway_s2=ReplaceNoCase("#driveway_s2#",",","","ALL")>
	
	<cfset driveway_collected_fee=#driveway_net_fee#+#driveway_s1#+#driveway_s2#>
	

<cfif #find_old_permit.ar_nbr# is not "">
<cfset driveway_fee = 0>
<cfset driveway_fee_discount = 0>
<cfset driveway_net_fee = 0>
<cfset driveway_s1 = 0>
<cfset driveway_s1 = 0>
<cfset driveway_collected_fee = 0>
</cfif>

	
	<cfset total_fee=#total_fee# + #driveway_fee#>
	<cfset total_fee_discount=#total_fee_discount# + #driveway_fee_discount#>
	<cfset total_net_fee=#total_net_fee# + #driveway_net_fee#>
	<Cfset total_s1=#total_s1#+#driveway_s1#>
	<cfset total_s2=#total_s2#+#driveway_s2#>
	<cfset total_collected_fee=#total_collected_fee#+#driveway_collected_fee#>
	
	
	<cfquery name="update_driveways" datasource="apermits_sql" dbtype="datasource">
	update driveway_details
	set
	driveway_fee=#driveway_fee#,
	driveway_fee_discount=#driveway_fee_discount#,
	driveway_net_fee=#driveway_net_fee#
