...
<cfset request.rslt_nc = "">		<!--- Resulting Neighborhood Council---> 	
...
<cftry>
	...
	<cfif getBCAI.recordcount gt 0><cfset request.rslt_bca_inspect_district = getBCAI.district></cfif>
		...
		<cfquery name="getNC" datasource="#pDS#">
	SELECT NAME FROM [navla].[dbo].[ags_neighborhood_councils] a
	where a.shape.STIntersects(geometry::STGeomFromText('POINT(#attributes.x# #attributes.y#)', 2229)) = 1
	</cfquery>
	<cfif getNC.recordcount gt 0><cfset request.rslt_nc = getNC.NAME></cfif>
