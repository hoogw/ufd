...
<!--- Started making changes for all the code below this line on 05/12/2017 --->

<!--- Get all apps with status = 'offerAccepted' that are not expired --->
...
and 
srr_info.offerAccepted_exp_dt >= #CreateODBCDate(now())#  <!--- This means that offer is not expired (60 days to submit all required permits --->
</cfquery>

<cfloop query="checkOfferAccepted"><!--- loop through all offerAccepted application to see if applicant submitted all required permits.  If applicant did not submit all required permits,  the status will be kept as 'offerAccepted' --->
...
<cfset request.newStatus = "offerAccepted"><!--- no change to status for now --->
...
<!--- Get all apps with status = 'offerAccepted' that are expired --->
<cfquery name="checkOfferAccepted_exp" datasource="#request.dsn#" dbtype="datasource">
SELECT
 srr_info.srr_id
 , srr_info.srrKey
 , srr_info.sr_number
...
srr_info.srr_status_cd = 'offerAccepted'
and 
srr_info.offerAccepted_exp_dt < #CreateODBCDate(now())#  <!--- This means that offer is  expired, applicant did not submit all required permits within 60 days --->
</cfquery>
...
<cfset request.srNum = #checkOfferAccepted_exp.sr_number#>
...
#request.serverRoot#/srr/public/submit_an_appeal1.cfm?srrKey=#checkOfferAccepted_exp.srrKey#
">
<cftry>
<!--- No need to send comments to customer --->
<cfif #request.production# is "p"><!--- 000 --->
...
<cfquery name="requiredPermitsSubmitted" datasource="#request.dsn#" dbtype="datasource">
SELECT
 srr_info.srr_id
 , srr_info.srrKey
 , srr_info.sr_number
...
<!--- <cfmail to="Essam.Amarragy@lacity.org" from="SRROverNightTask@lacity.org" subject="requiredPermitsSubmitted">
#requiredPermitsSubmitted.recordcount# requiredPermitsSubmitted Applications are currently in database.
</cfmail> --->



<!--- <cfdump var="#requiredPermitsSubmitted#" output="browser"> --->

<!--- <cfabort> --->

<cfloop query="requiredPermitsSubmitted">
<!--- Set Defaults --->
...
<cfif isdate(#requiredPermitsSubmitted.ApermitIssued_dt#)>
<cfset request.ApermitIssued = 1>
<cfset request.ApermitIssued_dt = #requiredPermitsSubmitted.ApermitIssued_dt#>
</cfif>
<!--- check if A-permit is issued --->
...
FROM  tree_info
where 
 tree_info.srr_id = #requiredPermitsSubmitted.srr_id#
</cfquery>
...
<cfif #checkTreeInfo.recordcount# is 1><!--- 1 --->
	...
	FROM  Tree_removal_permit 
	Where srr_id = #requiredPermitsSubmitted.srr_id#
	</cfquery>
...
<cfelseif #request.ApermitIssued# is 1 and #request.TrPermitRequired# is 0 and #request.TpPermitRequired# is 0>
	...
	<cfset request.requiredPermitsIssued_dt = #requiredPermitsSubmitted.ApermitIssued_dt#>
...
<cfelseif #request.ApermitIssued# is 1 and #request.TrPermitRequired# is 1  and  #request.trPermitIssued# is 1 and #request.TpPermitRequired# is 0>
	<cfset request.newStatus = "requiredPermitsIssued">
	<cfset request.requiredPermitsIssued_dt = #requiredPermitsSubmitted.ApermitSubmitted_dt#>
...
<cfset request.requiredPermitsIssued_exp_dt = dateAdd("d", 90, #request.requiredPermitsIssued_dt#)><!--- This is the date if construction is not completed, the application wire expire --->
<!--- <cfabort> --->
...
<cfset request.srNum = #requiredPermitsSubmitted.sr_number#>
...
#request.serverRoot#/srr/public/app_requirements.cfm?srrKey=#requiredPermitsSubmitted.srrKey#
">
<cftry>
<!--- No need to send comments to customer --->
<cfif #request.production# is "p"><!--- 000 --->
...
<cfset request.srCode = "15"> <!--- If the value is numeric then it will update the Reason Code. Otherwise is will update the Resolution Code --->
