...
, offer_paid_amt = 0
, record_history = record_history + '|Application moved from waiting list to the received list on (#dateformat(Now(),"mm/dd/yyyy")# at #timeformat(now(),"h:mm tt")#)  (Action taken by Automated Nightly Script).'
...
<cfelseif (#waitListed.prop_type# is "C" or #waitListed.prop_type# is "") and #request.balanceNow# gte 4000>
<cfquery name="updateStatus" datasource="#request.dsn#" dbtype="datasource">
update srr_info
set
srr_status_cd = 'received'
...
, offer_accepted_amt = 0
, offer_paid_amt = 0
, record_history = record_history + '|14 days have passed since application was found to be NOT Eligible. Appeal deadline was on  #dateformat(notEligibleTemp.notEligible_exp_dt,"mm/dd/yyyy")# (Action taken by Automated Nightly Script).'
...
, offer_accepted_amt = 0
, offer_paid_amt = 0
, record_history = record_history + '|Application Canceled on #dateformat(Now(),"mm/dd/yyyy")# at #timeformat(now(),"h:mm tt")#.  14 days have passed since BCA indicated that the site is already ADA compliant -  (Action taken by Automated Nightly Script).'
...
, offer_accepted_amt = 0
, offer_paid_amt = 0
, record_history = record_history + '|Application status has been updated to Offer Expired on(#dateformat(Now(),"mm/dd/yyyy")# at #timeformat(now(),"h:mm tt")#).  Applicant did not accept or decline the offer within 14 days -  (Action taken by Automated Nightly Script).'
...
, offer_accepted_amt = 0
, offer_paid_amt = 0
, record_history = record_history + '|Application status has been permanently  updated to Offer Declined on(#dateformat(Now(),"mm/dd/yyyy")# at #timeformat(now(),"h:mm tt")#).  Applicant cannot accept the offer anymore (14 days have passed since applicant declined the offer -  (Action taken by Automated Nightly Script).'
where srr_id = #notEligibleTemp.srr_id#
</cfquery>
...
and 
ApermitSubmitted_dt is not null
and
a_ref_no is not null
</cfquery>
...
, record_history = record_history + '|All required permits were submitted as of #dateformat(Now(),"mm/dd/yyyy")# at #timeformat(now(),"h:mm tt")# -  (Action taken by Automated Nightly Script).'
where srr_id = #checkOfferAccepted.srr_id#
</cfquery>
...
, record_history = record_history + '|All required permits were NOT submitted as of #dateformat(Now(),"mm/dd/yyyy")#.  Request Expired for not meeting the deadlines - (Action taken by Automated Nightly Script).'
where srr_id = #checkOfferAccepted.srr_id#
</cfquery>
...
, record_history = record_history + '|All required permits were Issued as of #dateformat(request.requiredPermitsIssued_dt,"mm/dd/yyyy")# -  (Action taken by Automated Nightly Script).'
where srr_id = #checkRequiredPermitsIssued.srr_id#
</cfquery>
