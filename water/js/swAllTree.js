...
var app = angular.module('app', ['ngAnimate', 'ngTouch', 'ui.grid', 'ui.grid.exporter', 'ui.grid.selection', '720kb.datepicker', 'ui.bootstrap']);
...
app.controller('MainCtrl', ['$scope', '$http','$interval', '$q', '$templateCache', 'uiGridConstants', function ($scope, $http, $interval, $q, $templateCache, uiGridConstants) {
               ...
               // +++++++++++++++++++++ ++++++++++++++++ ++++++ watering ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
						  ...
						  $scope.waiting = 'Double click on any row to see the row data';
																...
																var req = {
																	...
																	$http(req).then(
																		...
																		{	
																			...
																			// above only reload iFrame contect, not the parent index.cfm
																			// Need to reload both index.cfm and iFrame
																			console.log(window.location.href);  // iframe 
																			
																			console.log(window.top.location.href);  // index.cfm 
																			
																			window.top.location.href = "../index.cfm?iframe=" + window.location.href  ;
																			//window.location.href = "../index.cfm?iframe=";
									...
									{	
								                           ...
								                           // ==== bug fix ============== remove property	"$$hashKey":"uiGrid-0011"  ================							
															  ...
															  // ---------- must convert object = string before post, since this post object can not handle deep levle of next object, ----------------
																   ...
																   var _water_selected_url = url + "search/swAllTreeSelected.cfm";
					       ...
					       rowTemplate: rowTemplate(),  // --- double click row - modal show up 
							 ...
							 */
							 ...
							 // default
							  ...
							  { 
								...
								minWidth: 70,
								 ...
								 cellTemplate: '<div class="ui-grid-cell-contents" title="TOOLTIP"><a href=""  ng-click="grid.appScope.filterGrid(grid.appScope.editLink_site_no(grid, row), 5 )">{{grid.appScope.editLink_site_no(grid, row)}}</a></div>'
							 ...
							 minWidth: 80,
							   ...
							   cellTemplate: '<div class="ui-grid-cell-contents" title="TOOLTIP"><a href=""  ng-click="grid.appScope.filterGrid(grid.appScope.editLink_crm(grid, row), 6 )">{{grid.appScope.editLink_crm(grid, row)}}</a></div>'
							 ...
							 */
							 ...
							 {   name: 'Action',
							  ...
							  { 
								...
								cellTemplate: '<div class="ui-grid-cell-contents" title="TOOLTIP"><a href=""  ng-click="grid.appScope.filterGrid(grid.appScope.editLink_package_no(grid, row), 9 )">{{grid.appScope.editLink_package_no(grid, row)}}</a></div>'
							  ...
							  cellTemplate: '<div class="ui-grid-cell-contents" title="TOOLTIP"><a href=""  ng-click="grid.appScope.filterGrid(grid.appScope.editLink_cd(grid, row), 10 )">{{grid.appScope.editLink_cd(grid, row)}}</a></div>'
				...
				// -------- single filter -----------
					  ...
					  $scope.singleFilter = function( renderableRows ){
							...
							renderableRows.forEach( function( row ) {
							  ...
							  ['PROGRAM_TYPE', 'BOE_BID_PACKAGE', 'BOE_SITE', 'CD', 'TBM', 'LOCATION', 'SPECIES','CRM_NUMBER', 'PLANTING_DATE','SUB_POSITION'].forEach(function( field ){
