...
<cfset pifDS = "srr"> <!--- This is the datasource for the ssr_info table --->
<cfset pifTable = "srr_info"> <!--- This is the ssr_info table --->
    ...
    <cfif myla311Data.Response.NumOutputObjects neq 0>
      ...
      <cfloop array="#the311Records#" index="i">
			...
			<cfif i.Owner is "BSS" AND ucase(srFirstName) is not "ANONYMOUS" AND ucase(i.Status) is "OPEN"> 	
				...
				SELECT count(*) as cnt FROM #pifTable# WHERE sr_number = '#srNo#'
				</cfquery>
				...
				<cfif checkRecords.cnt is 0>
					...
					INSERT INTO #pifTable# (
						...
						[x_coord],
      					...
      					[longitude],
      					[latitude],
						<cfif trim(srFiles) is not "">[file_urls],</cfif>
						[tbm_grid],
						[council_dist],
