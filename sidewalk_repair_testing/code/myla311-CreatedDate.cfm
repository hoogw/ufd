...
<cfset createdDateStart = DateFormat(DateAdd("h", -6, Now()),"MM/dd/yyyy") & " " & TimeFormat(DateAdd("h", -6, Now()),"HH:mm:ss")> <!--- Start date set 6 Hours before Now() --->
...
<cfset pifDS = "navla_testing_spatial"> <!--- This is the datasource for the PIF_form table --->
<cfset pifTable = "pif_form"> <!--- This is the PIF_form table --->
  ...
  <cfelse>
    ...
    <cfif myla311Data.Response.NumOutputObjects neq 0>
      ...
      <cfloop array="#the311Records#" index="i">
			...
			<cfif i.Owner is "BSS" AND ucase(srFirstName) is not "ANONYMOUS" AND ucase(i.Status) is "OPEN"> 	
			
				<!--- s: Check if request already exists in the database and add if not there --->
				<cfquery name="checkRecords" datasource="#pifDS#">
				SELECT count(*) as cnt FROM #pifTable# WHERE sr_nbr = '#srNo#'
				</cfquery>
				
				<cfif checkRecords.cnt is 0>
					<!--- s: Insert Records into the PIF_Form table --->
					<cfquery name="insertRecords" datasource="#pifDS#">
					INSERT INTO #pifTable# (
						[ddate_submitted],
						[sr_nbr],
						[app_contact_name_nn],
						[app_address1_nn],
					...
					Values (
						...
						'#srTBM#',
						'#srCD#',
						'#preservesinglequotes(srCmts)#',
						0
					)
					</cfquery>
					<!--- e: Insert Records into the PIF_Form table --->
