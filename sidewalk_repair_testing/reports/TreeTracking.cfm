...
with goodtree( Location_No, TREE_REMOVAL_DATE, SPECIES, TREE_PLANTING_DATE, START_WATERING_DATE, END_WATERING_DATE, ACTION_TYPE, ADDRESS, TREE_BOX_SIZE   ) as (
	select vwHDRTreeList.Location_No, TREE_REMOVAL_DATE, ISNULL(SPECIES, 'unspecified'), TREE_PLANTING_DATE, START_WATERING_DATE, END_WATERING_DATE, ACTION_TYPE, vwHDRTreeList.ADDRESS, TREE_BOX_SIZE 
	from vwHDRTreeList
	inner join vwHDRAssessmentTracking a on vwHDRTreeList.Location_No = a.Location_No
	inner join vwHDRWorkOrders w on a.Package = w.Package
	where w.Notice_To_Proceed_Date is not null	
	and vwHDRTreeList.[TYPE] = 'BSS' and vwHDRTreeList.DELETED = 0
),
