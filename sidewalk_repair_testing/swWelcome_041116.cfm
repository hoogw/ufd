...
SELECT e4 + e6 + e8 + eb + q4 + q6 + q8 + qb + c4 + c6 + c8 + cb AS total FROM (SELECT 
...
SUM(dbo.tblEngineeringEstimate.EIGHT_INCH_CONCRETE_DRIVEWAY_AND_SIDEWALK__HIGH_EARLY_STRENGTH___QUANTITY) AS e8, 
SUM(dbo.tblEngineeringEstimate.PATTERNED_l_DECORATIVE_l_BRICK_SIDEWALK_QUANTITY) AS eb, 
...
SUM(dbo.tblQCQuantity.EIGHT_INCH_CONCRETE_DRIVEWAY_AND_SIDEWALK__HIGH_EARLY_STRENGTH___QUANTITY) AS q8, 
SUM(dbo.tblQCQuantity.PATTERNED_l_DECORATIVE_l_BRICK_SIDEWALK_QUANTITY) AS qb, 
...
CASE WHEN SUM(dbo.tblChangeOrders.EIGHT_INCH_CONCRETE_DRIVEWAY_AND_SIDEWALK__HIGH_EARLY_STRENGTH___QUANTITY) IS NULL 
THEN 0 ELSE SUM(dbo.tblChangeOrders.EIGHT_INCH_CONCRETE_DRIVEWAY_AND_SIDEWALK__HIGH_EARLY_STRENGTH___QUANTITY) END AS c8, 
CASE WHEN SUM(dbo.tblChangeOrders.PATTERNED_l_DECORATIVE_l_BRICK_SIDEWALK_QUANTITY) IS NULL 
THEN 0 ELSE SUM(dbo.tblChangeOrders.PATTERNED_l_DECORATIVE_l_BRICK_SIDEWALK_QUANTITY) END AS cb
