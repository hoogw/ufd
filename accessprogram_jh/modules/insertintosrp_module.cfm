		 ...
		 <cfif getSiteInfo.bss_assessed_dt is not "">
			...
			<cfif getID.recordcount is 0>
				<cfloop query="getSiteInfo">
					...
					<!--- s: Retrieve access request type --->
					<cfset sr_class = "NULL">
					<cfquery name="getClassification" datasource="#sDS#">
					SELECT id FROM tblClassificationTypes WHERE upper(value) = upper('#sr_access_barrier_type#')
					</cfquery>
					<cfif getClassification.recordcount gt 0>
						<cfset sr_class = getClassification.id>
					</cfif>
					<!--- e: Retrieve access request type --->
				    ...
				    [Complaints_No] = 1,
					[Classification] = #sr_class#, <!--- Insert Classification --->
				    [SR_Number] = '#sr_number#',
