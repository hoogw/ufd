...
<!---  ,dateDiff("d",dbo.ar_info.bss_to_srp_dt, getdate())as daysInQueue1 --->
  , case 
		when isDate(ar_info.BSS_Assessed_dt)=1  then dateDiff("d",BSS_Assessed_dt, getdate())
		else  dateDiff("d",  ar_info.BSS_TO_SRP_DT, getDate()) 
	End as daysInQueue1  
<!--- 	 , 	case 
		when isDate(BSS_Assessed_dt)=1  then dateDiff("d",dbo.ar_info.dod_to_bss_dt, BSS_Assessed_dt)
		else  <!--- dateDiff("d", dbo.ar_info.dod_to_bss_dt, getDate())  --->''
	End as daysInQueue1  
 --->
 
<!---  , 	case 
		when isDate(ar_info.BSS_TO_SRP_DT)=1  then dateDiff("d",dbo.ar_info.dod_to_bss_dt, ar_info.BSS_TO_SRP_DT)
		else  <!--- dateDiff("d", dbo.ar_info.dod_to_bss_dt, getDate())  --->''
	End as daysInQueue  
	 , 	case 
		when isDate(BSS_Assessed_dt)=1  then dateDiff("d",dbo.ar_info.dod_to_bss_dt, BSS_Assessed_dt)
...
<!---   process time for bss is when dod sends it to bss and bss has completed assessment  --->
...
where
	...
	and ar_info.ar_status_cd = 'pendingboereview'
...
<cfoutput>
<div class="subtitle">#unprocessed_investigations.recordcount# Applications Pending BOE Review<br>
...
<th>Comments</th>
<th>Status</th>
<th>How Old (Days)<br>for BSS to Process</th>
