...
if (document.form1.swbb_step2_comp[0].checked == false && document.form1.swbb_step2_comp[1].checked == false)
{
alert ("Please Select Yes or No!");
document.form1.swbb_step2_comp[0].focus();
return false;
}
// Validate Radio Buttons 
...
Select arKey from bssEstimate_bb
...
Select * from bssEstimate_bb
where arKey = '#request.arKey#'
</cfquery>

<cfquery name="swbb" datasource="#request.dsn#" dbtype="datasource">
select swbb_step2_comp, ar_status_cd
from ar_info
where arkey='#request.arkey#'
</cfquery>
...
<cfoutput>
<form action="control.cfm?action=spd_bb_estimate_action&#request.addtoken#" method="post" name="form1" id="form1" onSubmit="return checkForm();">
<input type="hidden" name="arKey" id="arKey" value="#request.arKey#">

<div class="formbox" style="width:345px;">

<h1>Sidewalk Block to Block Information</h1>
...
<cfquery name="readBSSEstimate" datasource="#request.dsn#" dbtype="datasource">
Select #readFields.FieldName#_Quantity as xValue from bssEstimate_bb
where arKey = '#request.arKey#'
</cfquery>
...
<td colspan="4">
	...
	Is Sidewalk Block to Block Assessment Completed? <br>
	<p>
	
		<cfif #swbb.swbb_step2_comp# is "">
	<input type="Radio" name="swbb_step2_comp" value="Y" id="swbb_step2_comp" style="width:65px;"<!---  required="true" --->> Yes
 	<input type="Radio" name="swbb_step2_comp" value="N" id="swbb_step2_comp" style="width:65px;" <!--- required="true" --->> No	

			<cfelseif #swbb.swbb_step2_comp# is "y">
				<input type="Radio" name="swbb_step2_comp" value="Y" id="swbb_step2_comp" style="width:65px;" checked > Yes
		 		<input type="Radio" name="swbb_step2_comp" value="N" id="swbb_step2_comp" style="width:65px;" <!--- required="true" --->> No	

			<cfelseif #swbb.swbb_step2_comp# is "N">
				<input type="Radio" name="swbb_step2_comp" value="Y" id="swbb_step2_comp" style="width:65px;"  > Yes
		 		<input type="Radio" name="swbb_step2_comp" value="N" id="swbb_step2_comp" style="width:65px;" checked> No	

	
	
	
	<!--- <cfelse>
		Sidewalk Block to Block Assessment Status: 	<cfif #swbb.swbb_step2_comp# is "y"> 
													Completed
												<cfelse> Not Completed
													<br>
													<br>
													Change Sidewalk Block to Block Assessment Status:
													<input type="Radio" name="swbb_step2_comp" value="Y" id="swbb_step2_comp" style="width:65px;"<!---  required="true" --->> Yes
 													<input type="Radio" name="swbb_step2_comp" value="N" id="swbb_step2_comp" style="width:65px;" <!--- required="true" --->> No	
												</cfif>
 --->											
...
<cfif #swbb.ar_status_cd# is "BSSAssessmentCompleted">
<!----do nothing---->
<cfelse>
