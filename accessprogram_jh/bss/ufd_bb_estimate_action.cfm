<cfparam name="request.treebb_step4_comp" default="">
...
<!--- 			<cfquery name="updateTree" datasource="#request.dsn#" dbtype="datasource">
				update ar_info
				set treebb_step4_comp = '#treebb_step4_comp#'
				where arKey = '#request.arKey#'
			</cfquery>
 --->
 			...
 			<cfquery name="updateTree" datasource="#request.dsn#" dbtype="datasource">
				update ar_info
				set treebb_step4_comp = '#request.treebb_step4_comp#',
				<cfif #request.treebb_step4_comp# is "y">
							record_history = ISNULL(record_history, '') + '|Non SAR Trees was marked as completed by #client.full_name# on #dateformat(now(),"mm/dd/yyyy")# at #timeformat(now(),"h:mm tt")#'
					<cfelse>
							record_history = ISNULL(record_history, '') + '|Non SAR Trees was updated by #client.full_name# on #dateformat(now(),"mm/dd/yyyy")# at #timeformat(now(),"h:mm tt")#'
					</cfif>
				where arKey = '#request.arKey#'
			</cfquery>
