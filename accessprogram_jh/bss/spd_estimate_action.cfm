<cfparam name="request.sw_step1_comp" default="">
...
<!--- 	<cfif isdefined("sw_step1_comp")>
		...
		<cfelse> --->
			<cfquery name="updateSw" datasource="#request.dsn#" dbtype="datasource">
				update ar_info
				set sw_step1_comp = '#request.sw_step1_comp#',
				<cfif #request.sw_step1_comp# is "y">
								record_history = ISNULL(record_history, '') + '|SAR was marked as completed by #client.full_name# on #dateformat(now(),"mm/dd/yyyy")# at #timeformat(now(),"h:mm tt")#'
<cfelse>
				record_history = ISNULL(record_history, '') + '|SAR was updated by #client.full_name# on #dateformat(now(),"mm/dd/yyyy")# at #timeformat(now(),"h:mm tt")#'
</cfif>
				where arKey = '#request.arKey#'
			</cfquery>
<!--- 		</cfif>
