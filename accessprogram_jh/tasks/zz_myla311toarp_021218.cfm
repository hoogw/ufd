...
<cfset queryMethodAPI = "#request.myLA311Root#/myla311_qa/QuerySRs?apikey=8lN5QbdIeZGNu1P7orjIIemsDwrA7WAR">
<cfif request.production is "p"><cfset queryMethodAPI = "#request.myLA311Root#/myla311router/mylasrbe/1/SANQueryPageSR"></cfif>
...
<cfset error_email = "nathan.neumann@lacity.org,essam.amarragy@lacity.org"> <!--- This is email address to get the error. Should be "essam.amarragy@lacity.org" --->
  ...
  <cfelse>
    ...
    <cfif myla311Data.Response.NumOutputObjects neq 0>
      ...
      <cfloop array="#the311Records#" index="i">
			...
			<cfset srMAddress = ""><cfset srMUnit = ""><cfset srMCity = ""><cfset srMState = ""><cfset srMZip = ""><cfset srMobilityName = ""><cfset srMobilityRelation = "">
			<cfif StructCount(i.ListOfLa311SidewalkAccess) gt 0>
				...
				<cfif arrayLen(the311SidewalkAccess) gt 0>
					...
					<cfset srMState = ucase(left(trim(the311SidewalkAccess[1].State),2))>
					<cfset srMZip = ucase(trim(the311SidewalkAccess[1].ZIP))>
					<cfset srMobilityName = ucase(trim(the311SidewalkAccess[1].FirstName & " " & the311SidewalkAccess[1].LastName))>
					<cfset srMobilityRelation = ucase(trim(the311SidewalkAccess[1].Relation))>
			...
			<cfset srMobilityName = replace(srMobilityName,"'","''","ALL")>
			<cfset srMobilityRelation = replace(srMobilityRelation,"'","''","ALL")>
			...
			<cfif i.Owner is "BOE" AND ucase(srFirstName) is not "ANONYMOUS" AND ucase(i.Status) is "OPEN" AND srType is "SIDEWALK ACCESS"> 	
				...
				<cfif checkRecords.cnt is 0>
					...
					INSERT INTO #arpTable# (
						...
						<cfif trim(srMobilityName) is not "">[sr_mobility_name],</cfif>
      					<cfif trim(srMobilityRelation) is not "">[sr_mobility_relation],</cfif>
					...
					Values (
						...
						<cfif trim(srMobilityName) is not "">'#preservesinglequotes(srMobilityName)#',</cfif>
      					<cfif trim(srMobilityRelation) is not "">'#preservesinglequotes(srMobilityRelation)#',</cfif>
						'#srBarrier#',
						'#srMethod#',						
						'#srMobility#'<!--- ,
						0 --->
...
<cfmail to="essam.amarragy@lacity.org" from="AR@lacity.org" subject="311 to Access Request ran successfully at #now()#">
311 to Access Request ran successfully at #now()#
</cfmail>
